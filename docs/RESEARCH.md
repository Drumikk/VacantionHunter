...
| Arbeidsplassen/NAV feed | <https://navikt.github.io/pam-stilling-feed/>; <https://arbeidsplassen.nav.no/vilkar-api> | Официальная непрерывная лента большинства публичных вакансий Норвегии (без FINN.no), Bearer JWT, `next_url`, ETag/Last-Modified, ACTIVE/INACTIVE; условия разрешают показ/анализ, требуют немедленных обновлений/удалений и ссылки на исходный отклик | `src/connectors/nav.js` + lifecycle-aware store |
...
4 августа 2026 года Arbeidsplassen/NAV проверен через официальный вращающийся experiment token. Один ограниченный проход получил HTTP 200, обработал 1 000 feed events и 77 активных заголовков; запрос `sykepleier` загрузил 10 detail records и дал 10 полных совпадений без warnings. Реализация сохраняет `next_url`, ETag/Last-Modified и write-ahead pending batch, удаляет INACTIVE и подтверждает cursor только после успешного обновления хранилища. Production token запрашивается у NAV после письменного принятия условий; публичный token не считается production credential.
