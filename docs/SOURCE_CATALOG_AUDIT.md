...
После текущего расширения оркестратор знает 48 независимо наблюдаемых источников при стандартной конфигурации: 26 API/RSS/policy-источников плюс 22 конкретные карьерные доски Greenhouse/Ashby/Lever/Recruitee. Каждый источник имеет отдельные health, timeout, retry и cooldown; падение одного не останавливает остальные.
...
| Агрегаторы и региональные API | 11 | Jooble + 8 стран Adzuna + Reed + SuperJob | Adzuna, Reed и SuperJob включаются после добавления ключей; число стран Adzuna влияет на дневную квоту |
