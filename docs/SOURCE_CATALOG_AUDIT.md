...
После текущего расширения оркестратор знает 44 независимо наблюдаемых источника при стандартной конфигурации: 22 API/RSS/policy-источника плюс 22 конкретные карьерные доски Greenhouse/Ashby/Lever/Recruitee. Каждый источник имеет отдельные health, timeout, retry и cooldown; падение одного не останавливает остальные.

| Слой | Подключено | Доступ | Примечание |
|---|---:|---|---|
| Государственные API | 3 | «Работа России» и JobTech без ключа; USAJOBS с ключом | Самые устойчивые и юридически прозрачные данные |
| Глобальные/remote API и RSS | 5 | Remotive, Arbeitnow, Remote OK, WWR, HN без пользовательского логина | Атрибуция сохраняется, HTML login scraping не используется |
| Агрегаторы | 9 | Jooble + 8 стран Adzuna по умолчанию | Adzuna поддерживает 16 стран текущего scope, но число стран влияет на дневную квоту |
| Международные организации | 1 | ReliefWeb с одобренным `appname` | Подключается одной переменной окружения |
| Публичные ATS работодателей | 22 boards | Без ключей | Greenhouse 5, Ashby 6, Lever 5, Recruitee 6 |
...
1. Один адаптер на ATS-платформу. Greenhouse, Ashby и Lever уже дают 16 отдельных источников. Следующие высокоэффективные семейства: Recruitee Careers Site API, Workable, SmartRecruiters и публичные career endpoints BambooHR. Для каждой компании хранится только slug и метаданные.
2. Официальные государственные API. Следующая очередь: France Travail, европейская EURES-интеграция, Canada Job Bank и национальные feeds, где опубликованы машинные интерфейсы и приемлемые условия.
3. API-ключи агрегаторов. Adzuna, Reed, SuperJob и похожие сервисы подключаются только после регистрации и проверки лицензии, квот и требований к атрибуции.
...
|---|---|---|---|
| 1 | Дополнительные проверенные Recruitee/Greenhouse/Ashby/Lever slugs | десятки/сотни карьерных досок существующими адаптерами | live-проверка каждой доски и allowlist географии |
| 2 | Generic RSS/Atom registry | десятки niche/remote/academic feeds | официальная ссылка на feed и условия републикации |
| 3 | France Travail, EURES и другие official APIs | национальное покрытие Европы | ключ/OAuth и подтверждённые квоты |
| 4 | Reed/SuperJob и расширение Adzuna до 16 стран | широкий aggregator coverage | ключи, атрибуция, приемлемая лицензия и контроль квоты |
