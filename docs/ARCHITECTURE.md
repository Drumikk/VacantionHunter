...
- Пользователь включает «следить»: запрос сохраняется в `data/watch-store.json`, после перезапуска снова участвует в фоновых обновлениях, а новые данные публикуются через SSE. Это локальная однопользовательская реализация; email/Telegram и multi-user delivery остаются отдельным opt-in модулем.
