...
Интерактивный поиск не ждёт весь fan-out. `POST /api/search/stream` отвечает NDJSON-потоком: `initial` немедленно содержит ранжированный snapshot локального `JobStore`, каждое `progress` сообщает завершение источника и при новых данных несёт обновлённую выдачу, `done` фиксирует итог и полный отчёт. Браузер отображает первый snapshot сразу и заменяет его по мере поступления. Новый пользовательский запрос отменяет предыдущий HTTP-поток; AbortSignal запрещает запуск оставшихся в очереди коннекторов, хотя уже начавшиеся безопасно завершаются.

Широкое международное покрытие реализуется сочетанием государственных API, агрегаторных API, официальных RSS и публичных ATS, а не эмуляцией пользовательского браузера. Jooble даёт широкий международный охват, «Работа России», JobTech, Arbeidsplassen/NAV, Job Market Finland и USAJOBS закрывают государственные базы, Remote OK/WWR/HN — remote и community-сегмент, а ATS дают первичные вакансии работодателей. Общая дедупликация объединяет пересечения. HH остаётся необязательным резервным источником. Одинаковые запросы к credential-based API кэшируются, секреты остаются только в окружении сервера.
