const RELOCATION_POSITIVE = /(?:relocation\s+(?:support|assistance|package|bonus|benefit)|relocat(?:e|ing|ion)\s+(?:support|assistance|package|provided|available|covered)|(?:cover|covered|covers|pay|paid|provide|provided|offer|offered|offering)\s+(?:for\s+|your\s+|the\s+)?relocation|visa\s+sponsorship|sponsor(?:ing|ed|s)?\s+(?:a\s+)?(?:work|employment|immigration)\s+(?:visa|authorization|permit)|work\s+(?:visa|permit)\s+sponsorship|housing\s+assistance|aide\s+(?:à|a)\s+(?:la\s+)?mobilit[ée]|aide\s+(?:au\s+)?d[ée]m[ée]nagement|prise\s+en\s+charge\s+du\s+d[ée]m[ée]nagement|(?:поддержк|компенсац|оплат)[а-яё\s-]*(?:релокац|переезд)|(?:визов|иммиграционн)[а-яё\s-]*(?:поддержк|спонсор)|предоставл[а-яё\s-]*жиль[ея])/iu;
...
const REMOTE_POSITIVE = /(?:\bremote(?:[- ]first|[- ]only)?\b|\bwork\s+from\s+home\b|\bwork\s+from\s+anywhere\b|\bhome[- ]based\b|\bworldwide\b|\bt[ée]l[ée]travail\b|travail\s+(?:à|a)\s+distance|удал[её]нн[а-яё]*\s+(?:работ|формат)|можно\s+удал[её]нн[а-яё]*|дистанционн[а-яё]*\s+работ|\bdistans(?:arbete)?\b|\bhemifrån\b)/iu;
