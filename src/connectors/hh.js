...
export function hhConnector(config) {
  ...
  return {
    ...
    async search(query) {
      ...
      const data = await fetchJson(`https://api.hh.ru/vacancies?${params}`, { timeoutMs: config.requestTimeoutMs, headers: { "HH-User-Agent": config.hhUserAgent }, userAgent: config.hhUserAgent, retries: 1, fetchImpl: config.fetchImpl || fetch });
