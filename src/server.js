...
const service = new JobService({ connectors: createConnectors(config), store, config });
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (req.method === "GET" && url.pathname === "/api/health") return json(res, 200, { ok: true, jobs: store.jobs.length, scope: config.sourceScope, sources: service.getSources() });
    if (req.method === "GET" && url.pathname === "/api/sources") return json(res, 200, service.getSources());
    if (req.method === "GET" && url.pathname === "/api/events") {
      res.writeHead(200, { "Content-Type": "text/event-stream", "Cache-Control": "no-cache", Connection: "keep-alive" });
      res.write(`event: ready\ndata: ${JSON.stringify({ jobs: store.jobs.length })}\n\n`);
      const onJobs = (payload) => res.write(`event: jobs\ndata: ${JSON.stringify(payload)}\n\n`);
      const onSource = (payload) => res.write(`event: source\ndata: ${JSON.stringify(payload)}\n\n`);
      service.on("jobs", onJobs); service.on("source", onSource);
      req.on("close", () => { service.off("jobs", onJobs); service.off("source", onSource); });
      return;
    }
